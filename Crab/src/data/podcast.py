...
def load_cat_emo_label(label_path, dtype, debug=False, emolist=None):
    if debug:
        print(f"Loading {dtype} data from {label_path} in debug mode (100 samples)")
        label_df = pd.read_csv(label_path, sep=",")
        cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
        ...
        cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    if emolist is not None:
        cur_labs = cur_df[emolist].to_numpy()
    else:
